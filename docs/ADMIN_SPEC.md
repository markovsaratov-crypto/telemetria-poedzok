...
> **Сервис:** Телемат («Телеметрия поездок») · **Актуально на:** 2026-09-17 (прод — v2.38.2, деплой 21:06 UTC; fix-паки ревью v2.38.1+v2.38.2, откат — ветка `backup/pre-fixpacks-2026-09-17`)
...
Ворчер — обработчик асинхронных `TrafficJob`. В песочнице и в продакшене запускается in-process через `instrumentation.ts` (один процесс с API — известное ограничение, см. TECHNICAL §17). Мини-сервис на Bun (порт 3001, `mini-services/worker`) — опциональная изолированная альтернатива, в проде НЕ развёрнут.
