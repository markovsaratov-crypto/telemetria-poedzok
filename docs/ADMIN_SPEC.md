# Спецификация администратора — «Телемат»
...
- [13. Ворчер (Worker)](#13-ворчер-worker)
  ...
  - [13.3. Обработка задачи (пайплайн)](#133-обработка-задачи-пайплайн)
...
Настоящий документ является исчерпывающей спецификацией администратора системы «Телеметрия поездок». Документ предназначен для специалистов, отвечающих за развёртывание, настройку, мониторинг и сопровождение системы в производственной среде.
...
bun run db:generate
# Применение схемы к БД (поля topologyHash и кэшированные метрики)
bun run db:push
```
...
Параметры конечного автомата `idle | moving | gap` для расчёта `MovingTime`, `IdleTime`, `GapTime`. Подробное обоснование порогов — в методологии, раздел 4.6.
...
Параметры CAP (Continuous Acceleration Profiling) для расчёта `EcoScore`. Полная формула — в методологии, раздел 7.3.
...
| /api/auth/me       | GET       | Текущая сессия; для user — личный ingestToken |
...
**Схема Session (поля кэша метрик — опциональные):**
...
> Поле `bearing Float?` активно используется. Все кэш-поля опциональны (`?`) — их отсутствие означает «не посчитано»; фактический расчёт происходит on-demand при `GET /api/sessions/[id]/stats`, после чего поля заполняются и переиспользуются до обновления точек.
...
**Схема БД (expand-only миграции; текущее состояние):**
...
| /api/admin/requeue | 10         | 60 сек   | Bearer/IP                |
| **read-скоп** (GET: /api/sessions, /api/sessions/{id}/stats, /events, /track, /api/stats/batch, /api/events/batch, /api/track/batch, /api/geocode/reverse; POST: /api/sessions/batch) | 240 | 60 сек | IP |
| Прочие /api/*     | 60         | 60 сек   | IP                       |
| /health            | без лимита | —        | —                        |

<!-- Таблица синхронизирована с proxy.ts. -->
...
Хронически пробочные участки (`HotspotSegments`, раздел 10.6 методологии) определяются по **75-му перцентилю** `TrafficSeverity` (не по среднему и не по `avg < 0.6`, ). Условие: `P75 < HOTSPOT_SEGMENTS_THRESHOLD` (0.5 по умолчанию).
...
### 13.3. Обработка задачи (пайплайн)

Алгоритм обработки одной `TrafficJob` — расширенный пайплайн с state machine и HMM:
...
3. **Независимый расчёт IdleTime.** `IdleTime` считается **независимо** (state machine), а не `Duration − MovingTime`. Разница уходит в `GapTime`.
...
- Схема применена к БД: `bun run db:push` добавил поля `topologyHash` на `Session` и `Route`, а также кэш-поля метрик на `Session` (`activeDuration`, `preTripIdle`, `postTripIdle`, `activeIdleTime`, `accelerationRms`, `jerkRms`, `speedConsistencyIndex`, `bearingConsistency`, `uTurnCount`, `turnCount`, `highSpeedCornering`, `sessionReliability`). Миграция expand-only — деструктивных операций нет.
...
> **Поездки:** `GET /api/trips` (список поездок, scope), `GET /api/trips/batch?ids=` (полные статы ≤50, TTL 30 c), `GET /api/trips/[id]` (детальная + фрагменты + план), `POST /api/admin/backfill-trips` (backfill истории, admin:heavy 1/час, аудит trip.backfill; параметр `planDays` — за сколько дней ставить план-джобы, дефолт 30). `GET /api/stats` — новые поля `totalTrips`/`todayTrips`.
...
| /api/sessions/export                      | GET              | Cookie/API_KEY      | CSV-экспорт списка сессий + план-факт          |
...
- Все env vars настроены (см. раздел 4, включая `MOVING_TIME_*`, `ECO_SCORE_*`, `HMM_*`, `ROUTE_TREND_*`, `HOTSPOT_SEGMENTS_*`, `ROUTE_ID_SNAP_GRID_M`).
...
- AlertManager правила импортированы (включая `hmm_mapmatching_fallback_rate`, `moving_time_control_sum_violations`, `eco_score_low_reliability_spike`).
