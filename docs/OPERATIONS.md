...
со спецификацией (актуальная — v2.31.0). Обновлять при изменениях.
Расположение на GitHub: https://github.com/markovsaratov-crypto/telemetria-poedzok/blob/main/docs/OPERATIONS.md

## Известные процедуры: кэш предрасчёта метрик (v2.31.0)

- **После рекалибровки корпуса EcoScore** (изменение env `ECO_SCORE_CAP_BASELINE` / существенный рост корпуса, см. METHODOLOGY.md §7.3) — **поднять `SESSION_CACHE_VERSION`** в `src/lib/session-cache.ts`: персистентный кэш v2.27 (`Session.statsCache/eventsCache/trackCache`) хранит базлайны МОМЕНТА записи, и без бампа версии батч-роут продолжит отдавать старые значения EcoScore, а поштучный роут — новые (MIN-12, конвенция зафиксирована в `src/app/api/stats/batch/route.ts`).
- **Первый заход на период после v2.31.0 пересчитает кэши** (`SESSION_CACHE_VERSION` 2 → 3: speedDistribution переведён на 6 бакетов §5.3, кап правдоподобной скорости унифицирован до 200 км/ч — меняет plausibilityScore §11.6): первый запрос «Всё время»/периода после деплоя будет медленным (live-пересчёт + write-through), последующие — из кэша.
...
**Статус устранения (выполнено агентом 01.09.2026):**
...
2. ✅ Git-история вычищена `git filter-repo` по путям `.env`,
   ...
   переклонировать.
3. ✅ Код (v2.11.0): новые GitHub-бэкапы создаются ТОЛЬКО приватными
