...
## 0. Инцидент C-1 (v2.11.0): публичные дампы БД — REQUIRED OWNER ACTION
...
`db/backups/prod-dump-2026-08-28…json`; (б) выгружен ПУБЛИЧНЫМИ ассетами GitHub
Releases (backup-*, проверено: анонимная загрузка даёт HTTP 200).

Что уже сделано кодом (v2.11.0): файл удалён из рабочего дерева и индекса git,
`.gitignore` закрывает `db/backups/*.json` и `*.db`; новые бэкапы НЕ содержат
`diag.ingest.raw` (сырые тела батчей).

Что должен сделать ВЛАДЕЛЕЦ (нужен GitHub-токен с правами admin:repo):
1. Удалить старые release-ассеты: Settings → Releases (или gh api) — все теги
   `backup-*` до даты очистки; либо перевести репозиторий в private.
2. Вычистить git-историю: `git filter-repo --path db/backups/prod-dump-2026-08-28T15-23-30-011Z.json --invert-paths` + force-push.
3. Ротация секретов (все значения были в публичном доступе): LOGIN_PASSWORD
   (владелец знает сам), API_KEY, INGEST_TOKEN, ADMIN_TOKEN, CRON_SECRET,
   SESSION_SECRET (env на Render), TWO_GIS_API_KEY (личный кабинет 2ГИС).
   После ротации: обновить SensorLogger URL (новый INGEST_TOKEN) и
   скрипты/закладки с токенами.
4. Перезаписать/удалить старые бэкапы в /tmp/backups (эфемерные — исчезнут
   при следующем рестарте Render сами).

Пока 1–3 не выполнены, считать ВСЕ токены скомпрометированными.
...
## 4. Диагностика канала приёма (ingest) — DIAG-1, v2.10.6; DIAG-2, v2.10.7; DIAG-3, v2.10.8; v2.11.0
...
- **v2.10.7 (DIAG-2): образец структуры `sample`** — при нераспознанном батче
...
- **v2.10.7 (DIAG-2): расширенный парсер SensorLogger** — координаты ищутся
...
- **v2.10.8 (DIAG-3): нативный формат Sensor Logger распознан.** Реальная
  структура батча (кейс 01.09 08:40, markov-iphone): корневой объект
...
- **v2.10.8 (DIAG-3): гистограмма сенсоров в `sample`** — для формата
...
- **v2.10.8 (DIAG-3): полный дамп нераспознанного батча** — Setting
  `diag.ingest.raw`: последнее тело запроса целиком (до 64 КБ, обрезка
  помечается `truncated`), исходы `no_gps`/`empty`. Отдаётся ТОЛЬКО по
...
- **v2.11.0:** инжест SensorLogger идемпотентен по `messageId` (таблица
