...
> **Статус на 17.09.2026 (после деплоя v2.38.2):** fix-паки задеплоены на прод
> (Render, CI success, откат — ветка `backup/pre-fixpacks-2026-09-17`), НО
> ротация секретов ещё НЕ выполнена — весь ранбук ниже остаётся обязательным
> к исполнению. Харднинг шлюза (вайтлист/лимиты/rate-limit) в коде воркера
> уже в репо, однако воркер на Cloudflare ещё не пердеплоен — до его деплоя
> работает старая версия (без вайтлиста).
