...
> **Статус на 18.09.2026 (раунд 19, v2.40.0):** директива владельца — ротация
> ВСЕХ токенов/секретов ОТЛОЖЕНА до завершения продуктовой фазы («сначала
> сделай идеальный продукт»). Ранбук ниже остаётся обязательным, порядок не
> меняется. Что добавилось в скоуп ротации с v2.40.0: **CRON_SECRET** теперь
> принимается ещё и d1-gateway-воркером (биндинг для §B5 cron-канала) — при
> ротации обновлять ОБЕ стороны (Render env + воркер-биндинг); **TURSO_AUTH_TOKEN**
> добавлен биндингом в воркер (только на время §T-DELTA миграции дельты —
> отозвать/удалить биндинг после статуса done через /admin/turso-migrate/status);
> **GITHUB_BACKUP_ENCRYPTION_KEY** сгенерирован и выставлен в Render env —
> durable-копии бэкапов снова шифруются (AES-256-GCM, TELMENC1). Фильтр
> git-истории ИСПОЛНЕН (force-push 18.09, 253 коммита переписаны, 8 секретов
> вычищены из блобов), но секреты всё равно считаются скомпрометированными
> (PR-refs GitHub держат старые коммиты до GC — тикет Support pending).
>
