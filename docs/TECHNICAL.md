...
**Версия:** 2.40.0 (18.09.2026 — на базе v2.39.1: §B5 Cron Triggers d1-gateway (планировщик вместо cron-сервисов Render, расписания 1:1 + worker-tick */1), §B1-complete алиас /api/ingest, §T-DELTA мигратор Turso-дельты (состояние в KV, /admin/turso-migrate), §B6 OpenNext (@opennextjs/cloudflare) — сайт собирается под CF Workers; Turso-инцидент 17-18.09 закрыт 18.09 08:10 UTC: env-пара D1_GATEWAY_* выставлена через Render API, prod снова на D1-шлюзе; откат — ветка backup/pre-fixpacks-2026-09-17-clean)
...
- Холодный старт: после ~15 мин простоя инстанс засыпает; первый запрос ждёт пробуждения до ~50 с. Смягчение: пинг `/api/keepalive` (публичный эндпоинт) не реже раза в 15 мин — инстанс остаётся тёплым в рамках 750 ч/мес free-плана; альтернатива — повышение плана. Инжест-батчи мобильного приложения переживают пробуждение (HTTP-клиент с ретраями), первый батч задерживается.
...
**Обоснование `TARGET_LOAD_RPM = 100`** (претензия «число не обосновано»): это
якорь расчёта rate-limits, а не измеренный лимит. Фактический пик нагрузки
одного пользователя: инжест 1 устройства (батч каждые 15 с ≈ 4 RPM) + UI-опросы
активной сессии (~3 эндпоинта / 15 с ≈ 12 RPM) ≈ 16 RPM. `TARGET_LOAD_RPM` × 5–6
запаса = 100 → `RATE_LIMIT_MAX_INGEST` = ceil(100 × 1,2) = 120 (env-валидатор
требует ingest-лимит ≥ 120). Проверено фактом: prod p95 API < 500 мс при
штатной нагрузке; полная нагрузочная валидация не проводилась (нет
нагрузочного стенда — осознанное ограничение, см. §17).
...
Схема применяется через `prisma db push` (миграционный движок Prisma для Turso/libsql в этой конфигурации не используется). С v2.32.0 в репозитории есть **базлайн-миграция** `prisma/migrations/0_init/migration.sql` — полная схема (11 таблиц), сгенерированная из `schema.prisma`. Назначение — трассируемость и ревью изменений, файл НЕ применяется автоматически.

Процедура изменения схемы (expand-only):

1. Правка `prisma/schema.prisma`.
2. Diff для ревью (расширение или разрушение?):
   `npx prisma migrate diff --from-migrations prisma/migrations --to-schema-datamodel prisma/schema.prisma --shadow-database-url file:./db/shadow.db --script`
   (или проще: `git diff prisma/schema.prisma` — политика читается глазами).
3. Политика **expand-only**: только новые поля/таблицы/индексы; удаление/сужение колонок — отдельная операция с переносом данных. Не-nullable новые поля — через `@default` (бэк-заполнение ленивое).
4. На стенде `npm run db:push` → проверка → для прода push с прод-кредами (`DATABASE_URL`+`TURSO_AUTH_TOKEN`).
5. Перед push — свежий бэкап (см. §14); после push — smoke `/health` + вкладки.

Автоматического энфорсмента expand-only НЕТ (нет CI-запуска против прод-БД — сознательно: push выполняет владелец вручную, diff из п.2 — ручной гейт).
...
| `GET /api/trips/batch?ids=` | api (read-скоп) | Полные статы ≤50 поездок одним запросом (конвейер trip-stats на срезе [spanStart, spanEnd] конкатенированного потока точек состава — запись, входящая в несколько поездок, отдаёт каждой только свои точки); ответ `{stats: TripStats[], missing: []}` сеется в кэш карточек; TTL 30 с; v2.38.0: in-memory кэш конвейера (отпечаток строки Trip + ключ базлайнов, §10.3а) — на повторных запросах точки не читаются вовсе; ids обрабатываются параллельными чанками по 8 |
...
### 10.3а. Статы поездок: кэш чтений D1 (v2.38.0)

**Проблема:** `/api/trips/batch` (полл вкладки «Поездки» каждые 30 с) на каждом
TTL-промахе ответа пересчитывал статы ВСЕХ поездок — чтение всех точек состава
(56К строк на вкладку; до ~7 млн строк/час при открытой вкладке — квота чтений
D1 free 5 млн/день выгорала за час использования).

**Решение** (`src/lib/trip-stats.ts`): ядро конвейера (computeSessionStats по
срезу состава + span-математика) кэшируется в памяти процесса (TTL 1 ч, LRU 128,
`ttl-cache.ts`, globalThis) и валидируется на КАЖДОМ запросе двумя ключами:

1. **Отпечаток строки Trip** — `sessionIds + startTime + endTime + spanStart +
   spanEnd + statsComputedAt`. Все пути изменений меняют отпечаток:
   инвалидации trip-grouping (NULL-ят `statsComputedAt`), live-продление окна
   (`extendTripOnPoints` двигает `spanEnd`), смена состава (`joinNewSessionToTrip`,
   recompute). Промах → полный пересчёт (как раньше) + запись кэш-полей `Trip.*`.
2. **Ключ корпус-калибровки EcoScore** — версия/размер/ставки базлайнов
   (4 знака): пересчёт калибровки пересчитывает и статы (EcoScore зависит от
   базлайнов).

**Что читается на hit-пути:** только свежий ПЛАН-ФАКТ — 1 строка `TrafficJob`
поездки (сознательно мимо кэша: маршрутный джоб может завершиться позже
расчёта статов — план появляется в карточке без инвалидаций) + строка Trip,
загруженная роутом. Итого 2 строки на поездку вместо ~3 700.

**Записи `Trip.*`** — только на реальном пересчёте (раньше — на каждый TTL-промах
ответа; экономия квоты записей D1). Сбой записи — non-fatal, отпечаток
кэша в этом случае строится по прежнему значению строки.

**Метрики:** `trip_stats_cache_hit_total` / `trip_stats_cache_miss_total`
(`/api/metrics`); стабильное состояние — hit ≫ miss (miss только после
деплоя/перестарта слота, изменений состава/окна и сдвигов калибровки).

**Корпус-калибровка EcoScore** (`src/lib/eco-corpus.ts`, §7.3 методологии):
перед full-sweep проверяется ПОДПИСЬ корпуса — агрегат `COUNT(*) +
SUM(pointCount)` по финализированным записям (`endTime IS NOT NULL`), одна
строка. Подпись меняют только финализация/soft-delete/backfill точек → sweep
выполняется только по делу. Состав корпуса сужен до финализированных записей:
live-запись растёт каждую секунду, её rates «дышат» — включение живых записей
в медиану было шумом; базлайны теперь стабильны внутри активной поездки.

**Инцидент 16.09.2026 и оценки эффекта — `docs/OPERATIONS.md` §5а.**
...
Хранение и поиск: live-лог и ограниченная история — в дашборде Render; долговременного ретENTION/поиска по логам НЕТ (осознанное ограничение free-плана; внешний log drain — платная опция). Компенсации: критические события оставляют след в `AuditLog` (БД, retention 3650 дней), алерты — Slack, состояние алертов — `_AlertState` (в бэкапах).
...
| `worker_stuck` | старейший pending > 30 мин ИЛИ pending > 50 в течение 10 мин | MIN(scheduledFor) по TrafficJob + серия снапшотов pending |

Просмотр: `GET /api/admin/alerts` (JSON). Срабатывание: `logger.warn` + метрики + Slack (`SLACK_WEBHOOK_URL`). Дедупликация: горящее правило уведомляется не чаще раза в `ALERT_DEDUP_COOLDOWN_MIN` (60 мин; состояние в `_AlertState`, переживает рестарт; гашение правила разоружает его — следующее срабатывание уведомит сразу). Ограничения — §17.

`worker_stuck`: счётчик pending при одном пользователе слабочувствителен (десятки заданий — норма), поэтому основной сигнал — возраст старейшего pending-задания: очередь не двигается дольше 30 минут.
...
| 3. GitHub draft-релиз | тот же дамп-файл как ассет приватного draft-релиза `backup-<date>-<time>` (checksum в теле релиза) + read-back drill (скачивание/sha256/парс/счёт строк — проверка восстановимости) | репозиторий GitHub | ежедневно 03:30 UTC (код в `POST /api/admin/backup`: дамп → аплоад того же файла → drill) + еженедельный крон ВС 04:00 как страховка + вручную |
...
Верификация: sha256 из ответа = checksum в BackupJob и в теле релиза; бэкап дополнительно самопроверяется перечитыванием (BACKUP_VERIFICATION_ENABLED). После каждого аплоада выполняется read-back drill: ассет скачивается обратно, sha256 сверяется, JSON парсится, счётчики строк сравниваются с дампом-источником (аудит-запись `backup.drill`, при провале — Slack).

### 14.3. Восстановление из дампа (RTO 5–15 мин)

`POST /api/admin/restore {backupId}` (локальный дамп) или `POST /api/admin/restore {source:"github", tagName?}` (durable-копия; без `tagName` — последний backup-релиз) — рабочая реализация (покрытие всех таблиц):

1. Локальный источник: дамп-файл в `/tmp/backups` инстанса (эфемерный!). GitHub-источник: ассет draft-релиза скачивается по API (Accept: octet-stream, токен в `GH_TOKEN`), sha256 сверяется с checksum из тела релиза, файл кладётся в `/tmp/backups` и создаётся completed-BackupJob (type=github) — работает в т.ч. сразу после рестарта/деплоя, когда /tmp пуст.
...
```bash
# restore из durable-копии GitHub (последний релиз)
curl -X POST https://poedzok.fun/api/admin/restore \
  -H "Authorization: Bearer <ADMIN_TOKEN>" \
  -H "Content-Type: application/json" \
  -d '{"source":"github"}'
# → {ok, restoredAt, backupId, source:"github:backup-<date>-<time>", tablesCount, ...}
# или конкретный релиз:
#   -d '{"source":"github","tagName":"backup-2026-09-13-040000"}'
```

### 14.4. RPO/RTO

- RPO: durable-копия (GitHub draft-релиз) — ежедневно 03:30 UTC (в том же прогоне, что и локальный дамп: код `POST /api/admin/backup`; расписание крон-сервисов Render не менялось — смена расписания в render.yaml требует ручного подтверждения blueprint-синка): окно потери ≤ 24 ч (раньше было ≤ 7 дней — еженедельный релиз + эфемерный ежедневный /tmp-дамп). Перед рискованными операциями (миграции, массовые удаления) — ручной `POST /api/admin/backup/github`. Read-back drill после каждого релиза проверяет восстановимость копии.
- RTO: 5–15 мин — restore-роут §14.3 работает из обоих источников, включая GitHub после рестарта инстанса. Полный ручной перенос в новую БД — 30–60 мин (runbook для случая «и Render, и Turso недоступны» — OPERATIONS §2).
...
2. **Restore — через API, но ручной запуск** (RTO 5–15 мин): `POST /api/admin/restore` покрывает все таблицы и работает из обоих источников — локальный `/tmp`-файл и GitHub draft-релиз (`{source:"github"}`, в т.ч. после рестарта инстанса). Read-back drill после каждого ежедневного GitHub-релиза проверяет копию на восстановимость (sha256 + парс + счёт строк). Остаточный риск: полный drill-restore в отдельную БД не автоматизирован — ручной прогон по runbook OPERATIONS §2.
...
6. **Slack-алерты с дедупликацией по кулдауну** (60 мин, `_AlertState`): горящее правило не спамится каждые 5 мин; гашение разоружает правило. Повторная доставка при сбое вебхука не «съедается» кулдауном (состояние ставится только после успешной доставки).
7. **Free-план Render**: cold start ~50 c, эфемерный `/tmp`. Смягчение cold start: внешний пинг `/api/keepalive` (публичный эндпоинт) не реже раза в 15 мин держит инстанс тёплым (720 ч/мес < лимита 750 ч free-плана); повышение плана убирает ограничение целиком.
8. **Регистрация открыта** (`REGISTRATION_ENABLED=true`): публичная саморегистрация создаёт аккаунты role=user; админ — только вручную в БД. Основной сценарий — по-прежнему single-user.
9. **Тайлы карты** — внешние сервисы (OSM и др.): зависимость доступности UI-карты от них.
10. **Автотесты есть, но только unit-уровень** (`vitest`, 50 тестов: круговая статистика §7.7, EcoScore-калибровка §7.3, SessionReliability §11.6, медианные фильтры, restore-core, дедуп алертов) + CI GitHub Actions (tsc/eslint/vitest на push/PR). Интеграционных/E2E-тестов с БД нет — регресс рантайма держится на прод-смоуке после деплоя (см. §15).
11. **Watchdog батч-статов (25 с) — страховка, а не решение производительности.** Медленный путь устранён предрасчётом KPI при завершении сессии + TTL-кэшами; watchdog остаётся как graceful degradation (карточки грузятся поштучно), чтобы регресс не ронял вкладку целиком.
12. **Live-сессия — polling 15 с, а не WebSocket/SSE.** Каденс совпадает с инжест-батчами приложения (~15 с): более частый опрос не даёт более свежих данных. SSE/WS не введены сознательно (единый прокси-гейт, PWA-фон, ноль новой инфраструктуры/точек отказа); при появлении нескольких одновременных live-наблюдателей — первый кандидат на пересмотр.
