...
| 3. GitHub draft-релиз | дамп-файл как ассет приватного draft-релиза `backup-<date>-<time>` (checksum в теле релиза) + read-back drill (скачивание/sha256/парс/счёт строк — проверка восстановимости) | репозиторий GitHub | ежедневно 04:00 UTC + вручную |
...
- RPO: durable-копия (GitHub draft-релиз) — ежедневно 04:00 UTC: окно потери ≤ 24 ч (раньше было ≤ 7 дней — еженедельный релиз + эфемерный ежедневный /tmp-дамп). Перед рискованными операциями (миграции, массовые удаления) — ручной `POST /api/admin/backup/github`. Read-back drill после каждого релиза проверяет восстановимость копии.
