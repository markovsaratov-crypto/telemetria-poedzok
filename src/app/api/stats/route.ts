...
// v2.40.5 (квота-M-14, аудит Task 22): TTL 60 с → 120 с. Проблема «TTL = интервал
// опроса»: фронт (useStats, POLL_BASE_INTERVAL_MS=60с) стучится ровно в момент
// истечения — кэш был почти ВСЕГДА cold, каждый poll = полный пересчёт лёгких
// счётчиков (Session/Trip/Route/TrafficJob JOIN ×3) ≈ 0,3–0,5 млн rows_read/день
// на одного активного зрителя. При TTL 120с каждый второй poll — попадание, а
// 304/ETag в окне не пересчитывает ничего. Лаг счётчиков дашборда ≤2 мин —
// осознанный трейд (данные инжеста обновляются в других запросах живой записи
// с их собственным поллингом 15с, мимо этого кэша). Тяжёлые части (totalPoints/
// heatmap) и так идут через rollup/edge-KV (§A1/§B2).
...
const STATS_CACHE = getTtlCache<StatsCacheEntry>("stats-dashboard", 120_000, 64);
