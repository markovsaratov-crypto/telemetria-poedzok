...
const CACHE = getTtlCache<{ tracks: Record<string, unknown>[]; missing: string[] }>("track-batch", 30_000);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.info("batch track computed", { requestId, requested: ids.length, returned: tracks.length, missing: missing.length });
