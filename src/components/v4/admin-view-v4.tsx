        ...
        <div className="doc-grid">
          ...
          <div className="doc-log">
            ...
            <ul>
              ...
              <li>
                <b className="c-plum">спека v2.18.0</b> гуру-ревью тотальной чистки (аудит трёх
                агентов, ~30 фиксов): <b className="c-plum">P0-1</b> — проверка подписи share-токенов
                была no-op (unawaited Promise: любой поддельный токен проходил на публичный роут);
                <b className="c-plum">P0-2</b> — <s>транзакционные вставки GpsPoint падали всегда</s>
                (GpsPoint не было в реестре таблиц с v2.16.0: /api/ingest, CSV- и ZIP-импорт
                были сломаны; SensorLogger выживал на своём пути); P0-3 — <s>db-слой был неявно
                any</s> (TS7022 self-reference), ошибки форм аргументов были невидимы — теперь
                типизирован, 132 скрытых ошибки пойманы и починены. Прочее: свежая роль admin из
                БД (а не из cookie), keyset-пагинация без потери строки, dead-экспорт → честный
                «failed» (диалог больше не опрашивает вечно), канонический полилайн §10.6
                <s>всегда fallback</s> (JSON.parse объекта Row), CSV без client_id — одна сессия
                на файл <s>вместо тысячи одно-точечных</s>, zip-бомба по факту распаковки,
                кардинальность метрик нормализована, конвейер статов — без двойного прогона
                state machine (×2 дешевле батч), <s>states[] (N строк!) в каждом payload</s> —
              ...
              <li>
                <b className="c-plum">спека v2.24.0</b> восстановление пропавших поездок — инцидент владельца
                09.09 («где две поездки за сегодня? SensorLogger их отдавал»): с 03.09 инжест-запросы
                с телефона не доходили до сервера (iOS ATS блокирует http://-URL; часть суток — 504
                через битый edge TurboFlare), данные оставались только в локальных логах SensorLogger.
                Карточка «Подключение телефона» теперь выдаёт личный URL всегда через
                <b>push.poedzok.fun</b> — стабильный одиночный edge (без round-robin 504) —
                и прямо предупреждает про https/ATS. Новая карточка <b>«Импорт из ZIP архива»</b>
                на вкладке «Поездки» (для role=user): экспорт логов SensorLogger → загрузка архива —
                сессии привязываются к аккаунту импортёра (раньше импорт был только в «Админе»).
                Поддержка TurboFlare починила второй edge (62.152.61.236: 504 → 200).
              </li>
              <li>
                <b className="c-plum">спека v2.23.0</b> изоляция данных пользователей — репорт владельца
                08.09: <s>новые зарегистрированные пользователи видели чужую (владельческую)
                статистику</s>. Модель владения: Session.userId (NULL = данные владельца;
                uuid = данные пользователя). Правила видимости (lib/scope.ts):
                <b>user → только свои</b>, owner/API_KEY → userId IS NULL (хозяйские),
                admin → всё. Заскоуплены все read-роуты (sessions/stats/routes/батчи/
                экспорты/шеринг/тренды/устройства/теги/рекорд скорости + ingestTrace
                скрыт для user), write-роуты ([id] notes/share/export/delete — чужая
                сессия = 404). Ингест: ЛИЧНЫЙ токен User.apiKey в ?token= — сессии
                привязываются к пользователю (корреляция сессий тоже в рамках владельца
                канала); глобальный INGEST_TOKEN — канал владельца, не изменился.
                CSV/ZIP-импорт привязывается к импортёру. Кэши: TTL-кэши батчей и
                рекорда скорости ключуются зоной видимости (анти-утечка через общий
                кэш). UI: вкладка «Админ» скрыта для role=user; карточка «Подключение
                телефона» с личным Push URL на вкладке «Поездки».
                <b>v2.22.0:</b> мобильная шапка (вкладки/⌘K/? скрыты &lt;768px).
                <b>v2.20.0:</b> публичная регистрация.
                <b>v2.21.0:</b> bullet-графики.
              </li>
              <li>
              ...
              <li>
                ...
                моё прочтение «батч»; эндпоинт удалён в v2.18.0 (0 потребителей). <b>v2.17.1:</b> corpus-калибровка EcoScore
                прогревается <b>фоном в воркере</b> (каждые 4 мин) — <s>холодный свип ~15–20 с платил
                первый запрос после старта инстанса</s>. <b>v2.17.2:</b> фикс гонки посева (шторм
                воспроизводился в момент прилёта батча — посев теперь синхронный в queryFn) +
                префетч на корне лейаута + дедуп период-агрегата через тот же ключ
              </li>
              <li>
