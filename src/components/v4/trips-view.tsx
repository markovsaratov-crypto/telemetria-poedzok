...
// v2.31.0 (MIN-11): 10 → 15 минут — порог легаси-склейки выровнен с серверными
// Trip (TRIP_SPLIT_SEC = 900 с): до этого фолбэк-режим «Поездок» склеивал строже
// сервера, а подписи обещали одно и то же «одну поездку».
const TRIP_MERGE_GAP_MS = 15 * 60_000;
...
const ECO_LOADING_TIP =
  "Плавность вождения (EcoScore) | Статы этой поездки ещё загружаются — подождите пару секунд | Зоны: 80+ плавно · 60–79 умеренно · 40–59 агрессивно · ниже 40 опасно";
  ...
  return (
    <div ref={ref}>
        ...
        >
          ...
          <div style={{ fontSize: 12, lineHeight: 1.6 }}>
            {/* v2.24.0: подсказка зависит от роли — для зарегистрированного
                пользователя вкладки «Админ» нет: карточка «Подключение телефона»
                вверху, «Импорт из ZIP» — в конце списка; владельцу/админу
      ...
      {/* v2.26.2: импорт ZIP для зарегистрированных пользователей — восстановление
          ...
          Перенесён ПОД список поездок по требованию владельца (12.09, скрин):
          раньше карточка закрывала собой список поездок. */}
      {userInfo && (
    ...
    <>
      ...
      <div className="card tsum">
        <div className="tsum-main">
          ...
          <b
            data-tip="Что считается одной поездкой | Записи с паузами короче 15 минут объединяются в одну поездку | Телефон иногда приостанавливает приложение, и запись делится на части — здесь они показаны вместе"
          >
      ...
      >
        ...
        >
          {/* v2.28.0: подпись зоны с числовой границей (как в bullet-чарте
              ...
              v2.34.0: значение с единицей — «52%» (запрос владельца: единообразие
              с плашкой «Плавность» на вкладке Аналитики). */}
          <b>{eco != null ? `${eco}%` : "—"}</b>
  ...
  return (
    <div className={`trip ${isOpen ? "open" : ""}`}>
      ...
      >
        ...
        >
          <b>{agg?.ecoAvg != null ? `${agg.ecoAvg}%` : "—"}</b>
          <small>
            {agg?.ecoAvg != null
    ...
    <div className="trip-body">
      ...
      <div className="stats-grid" style={{ marginTop: 0, marginBottom: 10 }}>
        ...
        <Stat
          ...
          tip="Время в движении | Всё время в движении: старт — от 5 км/ч, остановка — ниже 2 км/ч"
          label="В движении"
        />
        <Stat
      ...
      <div className="t-ev">
        Записи объединены автоматически: паузы между ними короче 15 минут.
      ...
      >
        ...
        >
          {/* v2.28.0: зона с числовой границей (как в bullet-чарте Аналитики);
              ...
              v2.34.0: значение с единицей — «52%» (единообразие с Аналитикой). */}
          <b>{eco != null ? `${eco}%` : "—"}</b>
