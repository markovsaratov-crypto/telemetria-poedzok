...
const schema = z.object({
  ...
  // v2.36.0 (кейс 15.09 «блипы»): РАЗРЕЖЕННОЕ ДВИЖЕНИЕ. Интервал тишины
  // > MOVING_TIME_GAP_SEC с перемещением ≥ этого порога (метры) — устройство
  // ФИЗИЧЕСКИ переместилось за время тишины (кейс: логгер деградировал и писал
  // по 1–2 точки раз в 6–10 мин; перемещения между блипами 90 м – 3,4 км при
  // accuracy 2–7 м — неоспоримое движение). Средняя скорость displacement/dt
  // проходит общий state machine (гистерезис 5/2 + debounce: dt > 30 c
  // подтверждается мгновенно; парковочный GPS-дрейф ниже порога движением
  // не становится). Дефолт 75 м — выше дрейфа стоящего телефона (~10–50 м),
  // ниже реальных медленных манёвров (выезд с парковки 90+ м).
  SPARSE_MOVE_MIN_M: z.coerce.number().positive().default(75),
  ...
  // v2.42.0 («Вариант 1»): TripCalc — промежуточное хранение расчётов
  // финальных записей (>24 ч): рендер-снапшоты трека/событий + метрики.
  // false (или 403 шлюза до его деплоя) — рендер-батчи живут на пути
  // v2.41.0 (Session-кэш + прореживание на лету), корректность не меняется.
  TRIP_CALC_ENABLED: z.string().default("true"),
