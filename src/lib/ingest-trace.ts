...
// а как выглядит location-запись — не видно). Дамп хранит тело С УСЕЧЕНИЕМ.
// v2.38.2 (ревью F32): кап уменьшен 64 КБ → 8 КБ (символов ≈ байт для ASCII-JSON).
// GDPR-линза ревью: сырые тела телеметрии (accelerometer + координаты) в общей
// таблице Setting — 8 КБ достаточно, чтобы увидеть структуру location-записи
// (кейс 01.09 решён первыми килобайтами батча), а объём хранимых
// псевдонимизированных персональных данных сокращён в 8 раз. Обрезка
// помечается truncated: true, полный размер — в bytes.
...
export interface IngestRawDump {
  ...
  route: IngestRoute;
  outcome: IngestOutcome;
  bytes: number; // полный размер тела (ДО усечения)
  truncated: boolean; // дамп обрезан по MAX_RAW_CHARS (v2.38.2/F32: 8 КБ)
 ...
 * (?ingestRaw=1 в /api/stats), чтобы не таскать 8 КБ в каждом ответе
 * (v2.38.2/F32: дамп обрезается при записи, не при чтении).
