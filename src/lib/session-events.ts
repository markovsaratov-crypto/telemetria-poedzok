// src/lib/session-events.ts — v2.19.0: ЕДИНЫЙ конвейер G-G/events (код
// перенесён ДОСЛОВНО из src/app/api/sessions/[id]/events/route.ts).
// Потребители: одиночный роут (тонкая обёртка) и батч-роут /api/events/batch —
// цифры совпадают побайтово (тот же паттерн, что session-stats.ts в v2.17.0).
//
// Возвращает payload ответа events-роута: maneuvers/gg/harshEvents/hscEvents/summary.
// Входные точки — уже загруженные из БД (сортировка по timestamp asc на стороне SQL).
...
export interface EventsInputPoint {
  lat: number;
  lon: number;
  timestamp: number;
  speed: number | null;
  ...
  // normalizeSessionSpeeds требует поле в типе; events-конвейер его не читает
  // (единственный роут исторически передавал altitude: null)
...
export interface SessionEventsPayload {
  ...
  deviceId?: string;
  maneuvers: { lat: number; lng: number; t: number; longA: number; latA: number; speed: number; bearing: number }[];
  gg: { points: { x: number; y: number }[]; rings: number[] };
  harshEvents: { lat: number; lng: number; type: "braking" | "acceleration"; longA: number; t: number; speed: number }[];
  hscEvents?: { lat: number; lng: number; t: number; turnDeg: number; speed: number }[];
  summary: {
...
export function computeSessionEvents(sessionId: string, deviceId: string, rawPoints: EventsInputPoint[]): SessionEventsPayload {
  if (rawPoints.length < 5) {
    ...
    return {
      ...
      summary: { accelerationRMS: 0, jerkRMS: 0, harshBraking: 0, harshAcceleration: 0, maneuvers: 0 },
    };
  }
